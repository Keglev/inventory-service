...
class AnalyticsControllerAggregateSecurityTest extends AbstractAnalyticsControllerSecurityTest {
    ...
    @Test
    void should_return_401_when_stock_value_is_requested_unauthenticated() throws Exception {
    ...
    @Test
    void should_return_200_when_stock_value_is_requested_authenticated() throws Exception {
        mockMvc.perform(get("/api/analytics/stock-value")
                        .param("start","2024-02-01")
                        .param("end","2024-02-28")
    ...
    @Test
    void should_return_401_when_stock_per_supplier_is_requested_unauthenticated() throws Exception {
        mockMvc.perform(get("/api/analytics/stock-per-supplier"))
               .andExpect(status().isUnauthorized());
    }

    @Test
    void should_return_200_when_stock_per_supplier_is_requested_authenticated() throws Exception {
        mockMvc.perform(get("/api/analytics/stock-per-supplier")
                        .with(user("u").roles(USER)))
               .andExpect(status().isOk());
    }
