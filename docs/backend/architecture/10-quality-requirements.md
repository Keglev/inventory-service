...
| Q-02 | Security — credential safety | The production image and Fly.io configuration are inspected | No wallet password is required at runtime (`cwallet.sso` auto-login); the DB schema credentials are injected as Fly secrets and appear in neither source nor image. See [ADR-0001](./09-decisions/adr-0001-oracle-wallet-autologin.md) |
...
| Q-06 | Reliability — error consistency | Any handled failure occurs (validation, not-found, conflict, access-denied, unexpected) | The body is exactly `{error, message, timestamp}` with `error == HttpStatus.name().toLowerCase()`; no correlation ID, no stack trace. See [ADR-0004](./09-decisions/adr-0004-http-status-as-envelope.md) |
