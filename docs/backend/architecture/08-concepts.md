...
| Audit attribution | `createdBy` is read from the security context (`SecurityAuditHelper.currentUsername()`), never from the request body | The OAuth2 principal carries `email` as its name attribute; unauthenticated callers (migrations, batch) attribute to `system`. A client is not trusted to state who created a row |
