 ...
 * - Five rows per page, paged with the same pagination bar as the tables
 *   (counter and arrows, no rows-per-page choice). The chart keeps the height
 *   of five rows on every page, so the card does not jump between pages.
 ...
 * ofMinimum,critical,low}; the pagination labels come from the theme locale.
 */
import * as React from 'react';
import { Card, CardContent, Typography, Skeleton, Box, TablePagination } from '@mui/material';
...
export default function LowStockMini() {
  ...
  // A refetch can shrink the list below the current page; fall back to the last one.
    ...
    <Card>
      <CardContent sx={{ py: 1.5, px: 2 }}>
        ...
        {q.isLoading ? (
          ...
          <>
            <Box sx={{ height: ROWS_PER_PAGE * ROW_HEIGHT + 8 }} data-testid="low-stock-chart">
              <Box sx={{ height: visible.length * ROW_HEIGHT + 8 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 1, mt: 1 }}>
              ...
              <TablePagination
                ...
                rowsPerPageOptions={[]}
                sx={{ '& .MuiTablePagination-toolbar': { minHeight: 0, pl: 0 } }}
              />
            </Box>
          </>
        )}
