 ...
 * @enterprise
 * - Used only inside the page to enforce client scoping by supplier,
 *   even if the backend ignores the filter and returns a wide list.
 */
...
export default function Analytics(): JSX.Element {
   ...
   * - If the BE ignores filters, we still apply client scoping in `itemOptions`.
   */
  const itemSearchQ = useQuery<ItemRef[]>({
    ...
    queryFn: async () => {
      ...
      if (filters.supplierId) {
        ...
        // NOTE: Your BE currently returns a wide list even with supplierId.
        // We enforce scoping below in `itemOptions`.
      }
  ...
  /**
   * Final options for the type-ahead:
   * - Start from the async results (global or supplier-scoped).
   * - If supplierId is set → filter again on the client by supplierId.
   * - Always apply the debounced text filter on the client too.
   * @enterprise This guarantees correctness even if the backend returns a wide list.
   */
  ...
  /**
   * Safeguard: if the currently selected item disappears from the available options
   * (supplier changed or query filtered it out), clear it to avoid stale state.
   */
  React.useEffect(() => {
    if (selectedItemId && !itemOptions.some(it => it.id === selectedItemId)) {
      setSelectedItemId('');
      // preserve typed text; the user is still searching
    }
  ...
  const priceQ = useQuery<PricePoint[]>({
    ...
    enabled: !!selectedItemId, // fetch only after explicit selection
  });
        ...
        <Card>
          <CardContent>
            ...
            >
              ...
              {/* Supplier-aware type-ahead (always text-capable) */}
